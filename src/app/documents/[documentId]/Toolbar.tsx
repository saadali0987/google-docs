...
const FontSizeButton = () => {
  ...
  const increment = () => {
    const newSize = editor?.getAttributes("textStyle").fontSize ? parseInt(editor?.getAttributes("textStyle").fontSize) + 1 : parseInt(fontSize) + 1;
...
const HighlightColorButton = () => {
  const { editor } = useEditorStore();
  const current = editor?.getAttributes("highlight").color || "#FFFFFF";
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="p-0">
        <SketchPicker color={current} onChange={onChange} />
      </DropdownMenuContent>
    </DropdownMenu>
  );
};
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="h-7 min-w-7 shrink-0 flex flex-col items-center justify-center rounded-sm hover:bg-neutral-200/80 px-1.5 overflow-hidden text-sm">
          ...
          <div className="h-0.5 w-full" style={{ backgroundColor: current }} />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="p-0">
        <SketchPicker color={current} onChange={onChange} />
